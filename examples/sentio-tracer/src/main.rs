...
use reth::rpc::types::{BlockId, BlockNumberOrTag, TransactionInput, TransactionReceipt, TransactionRequest};
use reth::{
    builder::{NodeBuilder, NodeHandle},
    providers::CanonStateSubscriptions,
    tasks::TaskManager,
};
use reth::revm::primitives::ruint::aliases::U256;
...
async fn main() -> eyre::Result<()> {
    ...
    let receipt: TransactionReceipt = EthTransactions::transaction_receipt(eth_api, hash).await?.unwrap();
    ...
    let estimated_gas = eth_api.estimate_gas(request.clone(), Some(BlockId::pending()), None).await?;
