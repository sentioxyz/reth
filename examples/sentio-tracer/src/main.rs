...
use alloy_rpc_types_trace::geth::{
    sentio::{FunctionInfo, SentioTracerConfig},
...
use reth::{
    ...
    primitives::Account,
    providers::{CanonStateSubscriptions, DatabaseProviderFactory},
    revm::primitives::ruint::aliases::U256,
    rpc::{
        api::{
            eth::{
                helpers::{Call, EthTransactions},
                RpcReceipt,
            },
            EthApiServer,
        },
        types::{
            BlockId, BlockNumberOrTag, TransactionInput, TransactionReceipt, TransactionRequest,
        },
    },
    tasks::TaskManager,
};
use reth_chainspec::ChainSpec;
use reth_db::{tables, test_utils::create_test_rw_db, transaction::DbTxMut};
use reth_node_core::{
...
async fn main() -> eyre::Result<()> {
    ...
    let node_config = NodeConfig::test()
        .with_dev(DevArgs { dev: true, block_max_transactions: Some(1), block_time: None })
    ...
    tx.put::<tables::PlainAccountState>(
        user1,
        Account { balance: U256::from(100000000000000_u64), ..Default::default() },
    )?;
    db.commit()?;
    println!("3");
    ...
    let hash = EthTransactions::send_transaction(
        eth_api,
        TransactionRequest {
            from: Some(user1),
            to: Some(TxKind::Create),
            input: TransactionInput { data: Some(weth_code.into()), ..Default::default() },
    ...
    let receipt: TransactionReceipt =
        EthTransactions::transaction_receipt(eth_api, hash).await?.unwrap();
    ...
    let GethTrace::SentioTracer(trace) =
        debug_api.debug_trace_transaction(hash, tracing_opt.clone()).await?
    else {
    ...
    let estimated_gas =
        eth_api.estimate_gas(request.clone(), Some(BlockId::pending()), None).await?;
    ...
    let opt = GethDebugTracingCallOptions { tracing_options: tracing_opt, ..Default::default() };
    let GethTrace::SentioTracer(mut trace_call_trace) =
        debug_api.debug_trace_call(request, Some(block_id), opt).await?
    else {
        return Err(eyre::eyre!("no trace found"));
    };
