...
use reth::{
    ...
    rpc::{
        api::{eth::helpers::EthTransactions, EthApiServer},
        types::{
...
async fn main() -> eyre::Result<()> {
    ...
    let dev_accounts = EthTransactions::accounts(eth_api);
    ...
    let hash = EthApiServer::send_transaction(
        eth_api,
        TransactionRequest {
            from: Some(user1),
    ...
    let GethTrace::SentioTracer(trace) =
        debug_api.debug_trace_transaction(hash, tracing_opt.clone().into()).await?
    else {
    ...
    let estimated_gas =
        eth_api.estimate_gas(request.clone(), Some(BlockId::pending()), None, None).await?;
    let gas_limit = estimated_gas;
    request.set_gas_limit(gas_limit.to());

    let hash = EthApiServer::send_transaction(eth_api, request.clone()).await?;
    let head = notifications.next().await.unwrap();
    let tx = head.tip().body().transactions().next().unwrap();
    assert_eq!(*tx.hash(), hash);
