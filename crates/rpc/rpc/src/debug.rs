    ...
    Eth: TraceExt,
{
    ...
    pub async fn debug_trace_block(
        ...
        let mut evm_env = self.eth_api().evm_env_for_header(block.sealed_block().sealed_header())?;
        evm_env.cfg_env.sentio_config = opts.sentio_config.clone();
    ...
    pub async fn debug_trace_transaction(
        ...
        self.eth_api()
            .spawn_with_state_at_block(block.parent_hash(), move |eth_api, mut db| {
                ...
                eth_api.replay_block_until(&mut db, &block, index)?;
                ...
                let mut inspector = DebugInspector::new(opts.tracing_options)
                    .map_err(Eth::Error::from_eth_err)?;
                let res =
